...
use godot::builtin::{Array, GString, Transform3D, Vector3, Vector3Axis};
use godot::classes::class_macros::private::virtuals::ZipReader::Aabb;
use godot::classes::{
    light_3d, Engine, GpuParticles3D, Node, OmniLight3D, ParticleProcessMaterial, Texture2D,
};
...
impl FireSpawner {
    ...
    pub fn set_fire_strength(&mut self, strenght: f32) {
        ...
        self.fire.set_amount_ratio(strenght);
        self.smoke.set_amount_ratio(strenght.max(0.2));

        // scale lifetime to strength
        let fire_strength = self.fire_lifetime * f64::from(strenght);
