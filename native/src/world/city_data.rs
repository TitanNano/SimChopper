...
fn get_valid_tile_slopes_from_neighbors<'a>(
    ...
    tilelist: &'a TileList,
) -> Vec<(Compass, &'a Tile, HashSet<&'static TerrainSlope>)> {
    tilelist
        ...
        .map(|(dir, neighbor_tile)| {
            ...
            if tile.coordinates == (127, 13) {
                logger::debug!(
                    "Neighbor for {:?} {:?}({:?}): {dir:?} {:?} {:?}({:?}) - {:?}",
                    rotation.normalize_slope(tile.terrain.slope),
                    tile.coordinates,
                    tile.altitude,
                    rotation.normalize_slope(neighbor_tile.terrain.slope),
                    neighbor_tile.coordinates,
                    neighbor_tile.altitude,
                    available_options
                )
            }
